    ...
    Template[options.template].rendered = function () {
            ...
            beforeLoadComplete: function (r, nr) {
                ...
                _.each(nr, function (item) {
                    ...
                    _.each(self.columns, function (opt) {
                        var val = AristosUtils.getValueForPosition(opt.map, item);
                        if (opt.type == 'float') {
                            val = parseFloat(val) || '-';
        ...
        if(options.saveState) {
            ...
            gridOptions.rendertoolbar = function (toolbar) {
                ...
                var $btnExport = $('<button class="btn btn-default btn-xs" style="margin: 0 5px;" id="btnExportXLS"><i class="fa fa-table"></i> Экспорт в Excel</button>');
                ...
                $btnExport.on('click', function(e){
                    e.preventDefault();
                    $(options.renderTo).jqxGrid('exportdata', 'xls');
                });
            };
        }
