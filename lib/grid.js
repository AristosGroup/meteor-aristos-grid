...
Meteor.ReportGrid.prototype.render = function (options) {
    ...
    Template[options.template].rendered = function () {
       ...
       // if (Session.get('ready')) {
            ...
            // columns.push({text:col.label,dataField:col.label, width:100});

            var columns = _.map(options.gridFields, function (opt) {
                opt.text = opt.getLabel();
                opt.dataField = opt.name;
                opt.width = 150;

                return  _.pick(opt, 'text', 'dataField', 'width', 'cellsformat', 'aggregates', 'pinned', 'cellsalign', 'cellsrenderer', 'filtertype', 'renderer');
