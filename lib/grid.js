    ...
    Template[options.template].rendered = function () {
        ...
        for (var c in self.columns) {
            ...
            columns.push(_.pick(opt,
                'text', 'dataField', 'width', 'height', 'align', 'cellsalign', 'cellsformat', 'aggregates', 'pinned',
        ...
        if(options.mode == 'server') {
                ...
                beforeprocessing: function (data) {
                    if(data.error) {
                        ...
                        if(data.distincts) {
                            ...
                            //$(options.renderTo).jqxGrid({ columns: columns });
                        }
            ...
            //local mode
            rows = null;
            $(options.renderTo).on('initialized', function () {
            ...
            Deps.autorun(function () {
                ...
                Meteor.http.get('/grid/export.json?limit=1000000&collection='
                    ...
                    $(options.renderTo).jqxGrid('hideloadelement');
                    try {
            ...
            gridOptions.rendertoolbar = function (toolbar) {
                ...
                $btnExport.on('click', function (e) {
                    ...
                    if(self.options.mode == 'server') {
                        bootbox.confirm("Внимание, на экспорт будет отправлена только текущая страница.<br>" +
                            "Отправить все данные?", function (result) {
                            if (result) {
                                //todo Реализовать экспорт в excel при работе с сервером
                                /*
                                $(options.renderTo).on("pagesizechanged", function (event)
                                {
                                    ...
                                    $(options.renderTo).jqxGrid('exportdata', 'xls', self.name, true);
                                    $(options.renderTo).off("pagesizechanged");
                                });
                                $(options.renderTo).jqxGrid({ pagesize: 1000000 });
                                $(options.renderTo).jqxGrid('exportdata', 'xls', self.name, true);
                                */
                            }
                        });
